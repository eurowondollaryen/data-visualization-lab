...
### Tableau로 표현 가능한 모든 차트 종류
#### 1. 막대 차트
- 막대 차트는 범주형 데이터를 비교하거나 단일 변수의 분포를 표시하는 데 유용합니다. 제품 범주, 지역별 판매 또는 설문 조사 응답과 같은 개별 데이터와 잘 작동합니다.

#### 2. 꺾은선형 차트
- 꺾은선형 차트는 시간 경과에 따른 추세 또는 연속 데이터를 표시하는 데 적합합니다. 일반적으로 주가, 판매 추세 또는 온도 변화를 시각화하는 데 사용됩니다.

#### 3. 산점도
- 산점도는 두 수치 변수 간의 관계를 탐색하는 데 효과적입니다. 데이터에서 상관 관계, 클러스터 또는 이상값을 식별하는 데 도움이 될 수 있습니다.

#### 4. 파이 차트
- 파이 차트는 데이터 세트에서 다양한 범주의 비율을 표시합니다. 시장 점유율, 비율 분석 또는 전체 구성을 표시하는 데 적합합니다.

#### 5. 영역 차트
- 영역 차트는 라인 차트와 유사하지만 색상으로 채워져 시간 경과에 따른 누적 데이터 또는 누적 값을 쉽게 시각화할 수 있습니다.

#### 6. 트리맵
- 트리맵은 다양한 제품 범주의 시장 점유율 또는 컴퓨터의 파일 크기 분포와 같은 계층적 데이터를 시각화하는 데 유용합니다.

#### 7. 히트 맵
- 히트 맵은 색상 강도를 사용하여 2차원 그리드에서 데이터 포인트의 밀도 또는 분포를 나타냅니다. 지리적 데이터, 사용자 행동 패턴 또는 데이터 매트릭스를 시각화하는 데 적합합니다.

#### 8. Gantt 차트
- Gantt 차트는 일반적으로 프로젝트 관리에서 작업, 일정 및 종속성을 표시하는 데 사용됩니다. 프로젝트에서 작업의 기간과 순서를 보여줍니다.

#### 9. Box-and-Whisker plot
- Box-and-whisker plot 사분위수, 중앙값 및 이상값을 사용하여 수치 데이터의 분포를 요약합니다. 통계 데이터를 분석하거나 분포를 비교하는 데 유용합니다.

#### 10. 폭포형 차트
- 폭포형 차트는 일련의 데이터 포인트에 대한 긍정적 및 부정적 변화의 누적 효과를 표시하는 데 유용합니다. 일반적으로 재무 분석, 예산 책정 또는 손익 계산서에 사용됩니다.

#### 11. 맵
- Tableau는 지리 공간 데이터를 시각화하기 위한 다양한 매핑 기능을 제공합니다. 지역 또는 지리적 좌표별로 데이터를 표시하기 위해 포인트 맵, 채워진 맵 및 등치 맵을 지원합니다.
